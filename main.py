...
def get_acceleration(electric_field_x, electric_field_y, atom_positions):
    ...
    for i in range(0, n_atoms):    
        ...
        y_forwards_weight = y_float_index
        y_backwards_weight = 1 - y_float_index
        a_x_pedestal = (e_charge/mass)*((x_backwards_weight*y_backwards_weight*electric_field_x[y_int_index, x_int_index]) + (x_backwards_weight*y_forwards_weight*electric_field_x[y_int_index+1, x_int_index]) + (x_forwards_weight*y_forwards_weight*electric_field_x[y_int_index+1, x_int_index+1]) + (x_forwards_weight*y_backwards_weight*electric_field_x[y_int_index, x_int_index+1]))
        a_y_pedestal = (e_charge/mass)*((x_backwards_weight*y_backwards_weight*electric_field_y[y_int_index, x_int_index]) + (x_backwards_weight*y_forwards_weight*electric_field_y[y_int_index+1, x_int_index]) + (x_forwards_weight*y_forwards_weight*electric_field_y[y_int_index+1, x_int_index+1]) + (x_forwards_weight*y_backwards_weight*electric_field_y[y_int_index, x_int_index+1]))
...
def Verlet_Step(atom_position, atom_veloc, atom_accel, dt, wave_freq, t_wave):
    for i in range(0, n_atoms):
    ...
    atom_accel = get_acceleration(new_field_x, new_field_y, atom_position)
    for i in range(0, n_atoms):
...
sample_rate = 10
while t <= t_f:  
    positions, velocities, accelerations, temp_field_x, temp_field_y, plot_potential, temp_charges = Verlet_Step(positions, velocities, accelerations, dt, inc_wave_freq, t)
